...
>NOTE: I have switched from using `pre-commit` to using `prek`, which is a drop-in replacement with improvements.

`prek` is used to manage the lifecycle of `git` pre-commit hooks for you.
...
Update `prek` hooks and pin them to their release SHA using:
